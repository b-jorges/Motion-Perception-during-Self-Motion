...
moments <- c(mean = 1,variance = (0.1)^2,skewness = 0, kurtosis = 10000^10)
DataFrame3 = data.frame(x = (rpearson(1000, moments = moments)),
                        y = rnorm(1000,1,0.1))
ggplot(DataFrame3, aes(x)) + ####just to get an idea of what this function looks like 
  geom_density() +
  coord_cartesian(xlim=c(0.7,1.3))

rpearson(55, moments = moments)
rnorm(55,1,0.05)
...
SimulatePsychometricFunction = function(ID,Motion,velH, reps){
  ...
  ###get where values from staircase are likely to be located. More around 1, so kurtosis real high
  moments <- c(mean = 1,variance = (0.1)^2,skewness = 0, kurtosis = 10000) 
